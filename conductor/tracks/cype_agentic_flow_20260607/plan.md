...
## Phase 1: Backend CYPE BC3 Parser & BigQuery Integration
- [ ] Task: Implement `GET /api/budget/cype-lookup` endpoint in FastAPI.
    - [ ] Write unit tests verifying BC3 file download, HTTP routing, and FIEBDC-3 parsing (Red Phase).
    - [ ] Implement `cype_parser.py` using `httpx` to download and parse dynamic `.bc3` files from CYPE's Asturias server.
