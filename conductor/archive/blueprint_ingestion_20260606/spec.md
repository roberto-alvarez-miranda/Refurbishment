...
6. **Authentication & Authorization:** The backend must authenticate users via Firebase Auth (e.g., extracting the UID from the `Authorization` header token) and use this context to securely call GCP APIs (Gemini, Cloud Storage) on the user's behalf or ensure multi-tenant data separation.
